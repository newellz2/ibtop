    ...
    for (index, line) in reader.lines().enumerate() {
        ...
        let guid = if guid_str.starts_with("0x") || guid_str.starts_with("0X") {
            ...
            match u64::from_str_radix(&guid_str[2..], 16) {
                Ok(val) => val,
                Err(e) => {
                    eprintln!("Warning: Failed to parse GUID '{}' as hex: {}. Skipping line.", guid_str, e);
                    continue;
                }
            }
        } else {
            ...
            match guid_str.parse::<u64>() {
                Ok(val) => val,
                Err(e) => {
                    eprintln!("Warning: Failed to parse GUID '{}' as decimal: {}. Skipping line.", guid_str, e);
                    continue;
                }
            }
        };
        ...
        let lid = match parts[2].trim().parse::<u16>() {
            Ok(val) => val,
            Err(e) => {
                eprintln!("Warning: Failed to parse LID '{}': {}. Skipping line.", parts[2].trim(), e);
                continue;
            }
        };
        ...
        let port_number = match parts[3].trim().parse::<i32>() {
            Ok(val) => val,
            Err(e) => {
                eprintln!("Warning: Failed to parse port number '{}': {}. Skipping line.", parts[3].trim(), e);
                continue;
            }
        };
